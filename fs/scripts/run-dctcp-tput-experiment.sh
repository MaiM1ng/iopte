...
num_runs=1
home="/users/yukino"
setup_dir=$home/iopte/fs/utils
exp_dir=$home/iopte/fs/utils/tcp
...
function cleanup() {
  ...
  ssh $uname@$client 'screen -S $(screen -list | awk "/\\.client_session\t/ {print \$1}") -X quit'
  ssh $uname@$client 'screen -S $(screen -list | awk "/\\.logging_session\t/ {print \$1}") -X quit'
  ssh $uname@$client 'screen -wipe'
  ssh $uname@$client 'sudo pkill -9 -f iperf'
  ## IOVA logging
...
for ((j = 0; j < $num_runs; j += 1)); do
  ...
  ssh $uname@$client 'screen -dmS client_session sudo bash -c "cd '$setup_dir'; sudo bash setup-envir.sh -i '$client_intf' -a '$client' -m '$mtu' -d '$ddio' --ring_buffer '$ring_buffer' --buf '$buf' -f 1 -r 0 -p 0 -e 1 -o 1; cd '$exp_dir'; sudo bash run-netapp-tput.sh -m client -a '$server' -C '$num_clients' -S '$num_servers' -o '$exp'-RUN-'$j' -p '$init_port' -c '$cpu_mask' -b '$bandwidth'; exec bash"'
  ...
  ssh $uname@$client 'screen -dmS logging_session sudo bash -c "cd '$setup_dir'; sudo bash record-host-metrics.sh -f 0 -t 1 -i '$client_intf' -o '$exp-RUN-$j' --type 0 --cpu_util 1 --retx 1 --pcie 0 --membw 0 --dur '$dur' --cores '$cpu_mask' ; exec bash"'
  ...
  ssh $uname@$client "sudo rm /dev/null; sudo mknod /dev/null c 1 3; sudo chmod 666 /dev/null"
  scp $uname@$client:$setup_dir/reports/$exp-RUN-$j/retx.rpt $setup_dir/reports/$exp-RUN-$j/retx.rpt
